...
p <- ggplot(data = merge_data) +
    ...
    geom_text(aes(x= Age, y = Male, label = round((Male*-1)/10000,0)),
              size = 3, vjust = 0.5, hjust = 1) +
    geom_text(aes(x= Age, y = Female, label = round(Female/10000,0)),
              size = 3, vjust = 0.5, hjust = 1) +
    ...
    scale_y_continuous(breaks = brks,
                       labels = lbls) +
    coord_flip() +
    labs(title = '2020년 대한민국 인구/유저 수 피라미드', x = '연령구간', y = '인구 수(백만)') +
    theme_clean(base_family='NanumGothic') +
    theme(plot.title = element_text(hjust = 0.5),
          axis.ticks = element_blank())
...
p2 <- p +
    geom_text(aes(x = Age, y = Male_User, label = round((Male_User*-1)/10000,0)),
              data = merge_data[merge_data$Male_User < -20000,],
              size = 3, vjust = 0.5, hjust = 1, color = 'blue') +
    geom_text(aes(x = Age, y = Female_User, label = round(Female_User/10000,0)),
              data = merge_data[merge_data$Female_User > 20000,], 
              size = 3, vjust = 0.5, hjust = -1, color = 'blue')
p2